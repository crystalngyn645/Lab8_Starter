    ...
    if ('serviceWorker' in navigator) 
    {
      ...
      window.addEventListener('load', async () => {
        try {
          ...
          console.log('Service worker registration failed:', serviceWorkerError);
        }
...
async function getRecipes() {
  ...
  let savedRecipeStorage = localStorage.getItem('recipes');

  if (savedRecipeStorage) {
    return JSON.parse(savedRecipeStorage);
  } 
  ...
  return new Promise(async (resolveRecipeData, rejectRecipeData) => {
  /**************************/
  ...
  //            declared above
    for (let currentRecipeURL of RECIPE_URLS) {
  ...
  //            function is using the async keyword so we recommend "await"
        let recipeResponse = await fetch(currentRecipeURL);
  ...
  //            "await" again
        let parsedRecipeObject = await recipeResponse.json();
  // A8. TODO - Add the new recipe to the recipes array
        fetchedRecipeList.push(parsedRecipeObject);
  ...
  //            resolve() method.
        if (fetchedRecipeList.length === RECIPE_URLS.length) 
        {

          saveRecipesToStorage(fetchedRecipeList);

          resolveRecipeData(fetchedRecipeList);
        }

  // A10. TODO - Log any errors from catch using console.error
      }catch (recipeFetchError) 
      {

        console.error(recipeFetchError);

        rejectRecipeData(recipeFetchError);
      }
    }
